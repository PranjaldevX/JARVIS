...
from PyQt5.QtGui import QIcon, QPainter, QMovie, QColor, QPixmap, QTextCharFormat, QFont, QImage, QPen, QTextBlockFormat
...
class ChatSection(QWidget):
    def __init__(self):
        ...
        layout.setContentsMargins(-10, 40, 100, 0)  # Fixed: Added bottom margin
    ...
    def loadMessages(self):
        ...
        with open(TempDirectoryPath("Responses.data"), "r", encoding="utf-8") as file:
            messages = file.read()
            if messages is None:  # Fixed: Proper None check
                pass
            elif len(messages) <= 1:
                pass
...
class CustomTopBar(QWidget):
    ...
    def mouseMoveEvent(self, event):
        if self.draggablev and self.offset:
    ...
    def showInitialScreen(self):
        """Switch to the initial screen, hiding any existing screen."""
        # Hide current screen if one exists
        if self.current_screen is not None:
            self.current_screen.hide()
            
        
        # Create and show new initial screen
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()  # Fixed from _init__ to __init__
    ...
    def initUI(self):
        ...
        message_screen = MessageScreen()  # Added missing = operator
        ...
        top_bar = CustomTopBar(self, stacked_widget)  # Added missing = operator
        self.setMenuWidget(top_bar)
        self.setCentralWidget(stacked_widget)

def GraphicalUserInterface():
    app = QApplication(sys.argv)
    window = MainWindow()  # Added missing = operator
    window.show()
    sys.exit(app.exec_())
